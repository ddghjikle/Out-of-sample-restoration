...
# --- Training dataset --- #
class TrainDataOutdoor(data.Dataset):
    def __init__(self, crop_size, train_data_dir,category):
        super().__init__()
        train_list = train_data_dir + 'train_input_file.txt'
        with open(train_list) as f:
            contents = f.readlines()
            input_names = [i.strip() for i in contents]

        train_list = train_data_dir + 'train_GT_file.txt'
        with open(train_list) as f:
            contents = f.readlines()
    ...
    def get_images(self, index):
        ...
        input = transform_input(input_crop_img)
        gt = transform_gt(gt_crop_img)

        # --- Check the channel is 3 or not --- #
        if list(input.shape)[0] is not 3 or list(gt.shape)[0] is not 3:
